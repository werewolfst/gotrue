		...
		Config: &conf.GlobalConfiguration{
			Sms: conf.SmsProviderConfiguration{
				...
				Aliyun: conf.AliyunProviderConfiguration{
					ApiKey:          "LTAIDDOjXslcuk43",
					ApiSecret:       "c9vOvFYH6TYDF9YPK6BRqhSfbDZTyC",
					SignName:        map[string]string{"arduino": "墨之坊", "tit": "blinker"},
					TemplateCode:    "SMS_62540347",
					Product:         map[string]string{"arduino": "arduino", "tit": "三塔"},
					DefaultSignName: "blinker",
				},
			},
		},
	}
...
func (ts *SmsProviderTestSuite) TestAliyunSendSms() {
	...
	err = aliyunProvider.SendSms(phone, message, "arduino")
	require.NoError(ts.T(), err)
}

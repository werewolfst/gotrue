...
func (a *API) sendPhoneConfirmation(ctx context.Context, tx *storage.Connection, user *models.User, phone, otpType string, smsProvider sms_provider.SmsProvider, channel string, sender string) error {
	config := a.config
	...
	*token = fmt.Sprintf("%x", sha256.Sum224([]byte(phone+otp)))
	...
	if serr := smsProvider.SendMessage(phone, message, channel, sender); serr != nil {
		*token = oldToken
		return serr
	}
